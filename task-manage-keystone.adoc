...
summary: 通过启用与Cloud Volumes ONTAP一起使用的订阅并请求更改订阅服务级别的承诺容量，在NetApp Console中管理您的Keystone订阅。请求服务级别的额外容量可为Cloud Volumes ONTAP系统提供更多存储空间。 
---
= 管理Keystone订阅
:hardbreaks:
:allow-uri-read: 
...
. 从NetApp Console菜单中，选择“管理 >Licenses and subscriptions”。
. 选择* Keystone订阅*。
. 对于要链接的订阅，请选择 image:icon-action.png["三个并排的点组成的图标"] 并选择 *Link*。
...
. 从NetApp Console菜单中，选择“管理 >Licenses and subscriptions”。
. 选择* Keystone订阅*。
. 对于要调整容量的订阅，请选择 image:icon-action.png["三个并排的点组成的图标"] 并选择 *View detail and edit*。
. 输入一个或多个订阅所请求的承诺容量。
. 向下滚动，输入请求的其他详细信息，然后选择 *Submit*。
...
. 从NetApp Console菜单中，选择“管理 >Licenses and subscriptions”。
. 选择* Keystone*。
. 对于要取消链接的订阅，请选择 image:icon-action.png["三个并排的点组成的图标"] 并选择 *Unlink*。

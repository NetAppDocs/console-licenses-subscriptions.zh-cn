...
---
= 管理 Cloud Volumes ONTAP 的基于容量的许可证
:hardbreaks:
:allow-uri-read: 
:nofooter: 
:icons: font
...
+
为了更好地了解 Essentials 套餐显示的容量，您应该熟悉计费的工作原理 https://docs.netapp.com/us-en/bluexp-cloud-volumes-ontap/concept-licensing.html#notes-about-charging["了解 Essentials 套餐的收费"^]。
...
* 如果您计划通过 AWS 市场中的独立 Cloud Volumes ONTAP 订阅收取 Cloud Volumes ONTAP 容量费用，Cloud Volumes ONTAP 系统必须使用受支持的基于容量的套餐（Essentials 或 Freemium）。独立列表不支持 Professional 套餐，并继续通过 NIS 订阅收费。选择所需套餐后，使用 NetApp Console 中的*账单首选项*选择用于 Cloud Volumes ONTAP 账单的市场订阅。
...
* *高级使用情况*：此报告汇总了您帐户中消耗的总容量，并按计费方式（预先提交的许可证与按需付费）进行细分。其中包含以下信息：
+
[cols="1,2"]
|===
| 列 | 描述 
...
| 总消耗容量 | 帐户中所有 Cloud Volumes ONTAP 系统的总配置容量。 


| 预先承诺的总容量 | 您从 NetApp 购买的总许可容量，包括 BYOL 许可证和 Marketplace 合同。此值为 BYOL 总容量与 Marketplace 合同总容量之和。 


| BYOL 总容量 | 直接从 NetApp 购买的自带许可证 (BYOL) 所涵盖的预承诺容量部分。 


| 市场合同总容量 | 通过云提供商市场购买的年度市场合同或私人优惠所涵盖的预承诺容量部分。 


| PAYGO 总容量 | 任何预先承诺的许可证未涵盖的已消耗容量部分，按即用即付（PAYGO）每小时市场费率计费。 
|===
* *Cloud Volumes ONTAP 包使用情况*：此报告提供与高级使用情况报告相同的容量明细，但按许可包组织。其中包含以下信息：
+
[cols="1,2"]
|===
| 列 | 描述 
...
| 总消耗容量 | 此包在您帐户中的所有 Cloud Volumes ONTAP 系统上配置的总容量。 


| 预先承诺的总容量 | 您从 NetApp 购买的此套餐的总许可容量，包括 BYOL 许可证和 Marketplace 合同。此值为 BYOL 总容量与 Marketplace 合同总容量之和。 


| BYOL 总容量 | 此软件包的预先承诺容量部分，由您直接从 NetApp 购买的自带许可证（BYOL）所覆盖。 


| 市场合同总容量 | 通过云提供商市场购买的年度市场合同或私人优惠所涵盖的此套餐的预承诺容量部分。 


| PAYGO 总容量 | 此套餐的已用容量中不受任何预先承诺的许可证保护的部分，按即用即付（PAYGO）每小时市场费率计费。 
|===
...
| 系统 ID 和名称 | Cloud Volumes ONTAP 系统的唯一标识符 (UUID) 和显示名称。 


| 云 | 部署系统的云提供商（例如 AWS、Azure 或 Google Cloud）。 


| NetApp帐户 ID | 与系统关联的 NetApp 帐户的标识符。 


| 系统配置 | 系统的部署配置（例如，单节点或 HA 对）。 


| SVM 名称 | Cloud Volumes ONTAP 系统中 Storage Virtual Machine (SVM) 的名称。 


| 预配置容量 | 在 SVM 上配置的总容量。 


| 充电容量汇总 | 为 SVM 实际计费的容量。每个 SVM 都有最小可计费容量（默认情况下为 4 TiB，但这可能因账户而异）。如果 SVM 的已配置容量等于或低于该最小值，则此值将向上舍入到最小值；否则，它等于已配置的容量。 


| 市场计费条款 | 关联平台订阅的账单周期（例如，按小时或按年）。 


| Cloud Volumes ONTAP软件包或功能 | 与 SVM 关联的许可包或功能（例如 Essentials 或 Professional）。 


| 收费 SaaS 市场订阅名称 | 用于计费的市场订阅的名称。 


| 收费 SaaS 市场订阅 ID | 用于计费的平台订阅的唯一标识符。 


| 工作负载类型 | 指示 SVM 的卷是*主卷*（读/写卷），主动用于生产数据，还是*辅助卷*（DP 卷），数据保护目标卷，例如用于备份和复制的 SnapMirror 副本。 
|===
...
| 系统 ID 和名称 | 包含此卷的 Cloud Volumes ONTAP 系统的唯一标识符 (UUID) 和显示名称。 


| SVM 名称 | 包含卷的Storage Virtual Machine (SVM)的名称。 
